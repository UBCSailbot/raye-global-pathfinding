...
glm::vec3 ArcBall::mouseOnSphere(const glm::vec3 &tscMouse) {
  ...
  glm::float_t mag = glm::length(glm::dot(ballMouse, ballMouse));
  if (mag > 1.0) {
...
void ArcBall::beginDrag(const glm::vec2 &msc) {
  ...
  // Normal 'begin' code.
  mVDown = glm::vec3((mScreenToTCS * glm::vec4(msc.x, msc.y, 0.0f, 1.0))[3]);
}

void ArcBall::drag(const glm::vec2 &msc) {
  // Regular drag code to follow...
  mVNow = glm::vec3((mScreenToTCS * glm::vec4(msc.x, msc.y, 0.0, 1.0))[3]);
